import { ThemeToggleButton } from '@/components/ui/theme-toggle';
import { SidebarTrigger } from '@/components/ui/sidebar';
import {
  DashboardModeToggle,
  type DashboardMode,
} from '@/components/dashboard-mode-toggle';

interface DashboardHeaderProps {
  /** Which dashboard is currently being viewed — drives the toggle state. */
  currentMode: DashboardMode;
  /** Org slug for the finance route. Null = user has no primary org yet. */
  orgSlug: string | null;
  /**
   * Render the sidebar hamburger on the left. Only valid when the page is
   * wrapped in a SidebarProvider (currently just the translation dashboard).
   */
  showSidebarTrigger?: boolean;
}

export function DashboardHeader({
  currentMode,
  orgSlug,
  showSidebarTrigger = false,
}: DashboardHeaderProps) {
  return (
    <header className="border-b border-[rgba(128,128,128,0.3)] bg-background">
      <div className="flex h-16 items-center justify-between px-6">
        {/* LEFT: Hamburger menu (mobile only) + Logo */}
        <div className="flex items-center gap-4">
          {showSidebarTrigger && <SidebarTrigger className="md:hidden" />}
          <span className="text-xl font-bold tracking-tight text-slate-900 dark:text-white">
            bayaan.ai
          </span>
        </div>

        {/* CENTER: Dashboard mode toggle */}
        <DashboardModeToggle currentMode={currentMode} orgSlug={orgSlug} />

        {/* RIGHT: Theme toggle */}
        <ThemeToggleButton start="top-right" />
      </div>
    </header>
  );
}
