...
export default async function MosqueAdminDashboard({ params }: PageProps) {
  ...
  const [totalResult, monthResult, campaignsResult, productsResult, recentResult] = await Promise.all([
    donationsActive
      ? supabaseAdmin
          .from('transactions')
          ...
          <Card className="mb-8 border-amber-200 bg-amber-50/50 dark:border-amber-800 dark:bg-amber-900/10">
            <CardContent className="flex items-center gap-4 py-6">
              ...
              <div className="flex-1">
                ...
                <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">
                  Complete your merchant onboarding to start receiving donations directly into your bank account.
                </p>
              </div>
              <Link
                href={`/mosque-admin/${organization.slug}/settings`}
              ...
              <Card>
                ...
                <CardContent>
                  <p className="text-xs text-slate-500 dark:text-slate-400">
                    {(totalResult as { count?: number }).count ?? 0} paid donations
                  </p>
                </CardContent>
              </Card>
              <Card>
                ...
                <CardContent>
                  <p className="text-xs text-slate-500 dark:text-slate-400">
                    {(monthResult as { count?: number }).count ?? 0} donations this month
                  </p>
                </CardContent>
              </Card>
            </>
