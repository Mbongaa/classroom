...
/**
 * Session-based Learning Page
 *
 * Generates AI learning content from session transcriptions — no recording required.
 * URL format: /learn/session/[sessionId]?lang=en
 */
export default function SessionLearningPage() {
  ...
  const fetchLearningContent = useCallback(async () => {
    ...
    try {
      const response = await fetch(`/api/sessions/${sessionId}/learning-content`, {
        method: 'POST',
      ...
      console.error('[Session Learning Page] Failed to load content:', err);
  ...
  }, [sessionId, targetLanguage, fetchLearningContent]);
